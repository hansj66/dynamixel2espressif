...
bool 
Dynamixel2Espressif::setModelNumber(uint8_t id, uint16_t model_number)
{
  bool ret = false;
...
uint16_t Dynamixel2Espressif::getModelNumber(uint8_t id)
{
...
bool Dynamixel2Espressif::setID(uint8_t id, uint8_t new_id)
{
  return writeControlTableItem(ControlTableItem::ID, id, new_id);
}

bool Dynamixel2Espressif::setProtocol(uint8_t id, float version)
{
  uint8_t ver_idx;
...
bool Dynamixel2Espressif::setBaudrate(uint8_t id, uint32_t baudrate)
{
...
/* Commands for Slave */
bool Dynamixel2Espressif::torqueOn(uint8_t id)
{
  return setTorqueEnable(id, true);
}

bool Dynamixel2Espressif::torqueOff(uint8_t id)
{
  return setTorqueEnable(id, false);
}

bool Dynamixel2Espressif::setTorqueEnable(uint8_t id, bool enable)
{
  return writeControlTableItem(ControlTableItem::TORQUE_ENABLE, id, enable);
}

bool Dynamixel2Espressif::ledOn(uint8_t id)
{
  return setLedState(id, true);
}

bool Dynamixel2Espressif::ledOff(uint8_t id)
{
  return setLedState(id, false);
}

bool Dynamixel2Espressif::setLedState(uint8_t id, bool state)
{
  bool ret = false;
  uint16_t model_num = getModelNumberFromTable(id);
...
//TODO: Simplify the code by grouping model numbers.
bool Dynamixel2Espressif::setOperatingMode(uint8_t id, uint8_t mode)
{
  bool ret = false;
  uint16_t model_num = getModelNumberFromTable(id);
...
bool Dynamixel2Espressif::setGoalPosition(uint8_t id, float value, uint8_t unit)
{
...
float Dynamixel2Espressif::getPresentPosition(uint8_t id, uint8_t unit)
{
  if(unit != UNIT_RAW && unit != UNIT_DEGREE)
    return 0.0;
...
bool Dynamixel2Espressif::setGoalVelocity(uint8_t id, float value, uint8_t unit)
{
...
float Dynamixel2Espressif::getPresentVelocity(uint8_t id, uint8_t unit)
{
  if(unit != UNIT_RAW && unit != UNIT_PERCENT && unit != UNIT_RPM)
    return 0.0;
...
bool Dynamixel2Espressif::setGoalPWM(uint8_t id, float value, uint8_t unit)
{
...
float Dynamixel2Espressif::getPresentPWM(uint8_t id, uint8_t unit)
{
  if(unit != UNIT_RAW && unit != UNIT_PERCENT)
    return 0.0;
...
bool Dynamixel2Espressif::setGoalCurrent(uint8_t id, float value, uint8_t unit)
{
...
float Dynamixel2Espressif::getPresentCurrent(uint8_t id, uint8_t unit)
{
  if(unit != UNIT_RAW && unit != UNIT_PERCENT && unit != UNIT_MILLI_AMPERE)
    return 0.0;
...
bool Dynamixel2Espressif::getTorqueEnableStat(uint8_t id)
{
  bool ret = false;
...
int32_t Dynamixel2Espressif::readControlTableItem(uint8_t item_idx, uint8_t id, uint32_t timeout)
{
  int32_t ret = 0;
  uint16_t model_num = getModelNumberFromTable(id);
...
bool Dynamixel2Espressif::writeControlTableItem(uint8_t item_idx, uint8_t id, int32_t data, uint32_t timeout)
{
  bool ret = false;
  uint16_t model_num = getModelNumberFromTable(id);
...
int32_t Dynamixel2Espressif::readControlTableItem(uint16_t model_num, uint8_t item_idx, uint8_t id, uint32_t timeout)
{
...
bool Dynamixel2Espressif::writeControlTableItem(uint16_t model_num, uint8_t item_idx, uint8_t id, int32_t data, uint32_t timeout)
{
  bool ret = false;
  ControlTableItemInfo_t item_info;
...
uint8_t
Dynamixel2Espressif::getModelNumberIndex(uint16_t model_num)
{
  uint8_t i, ret = 0xFF;
...
uint16_t Dynamixel2Espressif::getModelNumberFromTable(uint8_t id)
{
  uint8_t idx;
  uint16_t model_num;
...
float Dynamixel2Espressif::readForRangeDependencyFunc(uint8_t func_idx, uint8_t id, uint8_t unit)
{
  float ret = 0;
  int32_t recv_data = 0;
  uint16_t model_num = getModelNumberFromTable(id);
...
bool Dynamixel2Espressif::writeForRangeDependencyFunc(uint8_t func_idx, uint8_t id, float value, uint8_t unit)
{
  bool ret = false;
  int32_t data = 0;
  uint16_t model_num = getModelNumberFromTable(id);
  ...
  if(model_num != UNREGISTERED_MODEL){
    ESP_LOGI(TAG, "Registered model");
    item_info = getModelDependencyFuncInfo(model_num, func_idx);

    if(item_info.item_idx == ControlTableItem::LAST_DUMMY_ITEM)
    {
      ESP_LOGE(TAG, "ControlTableItem::LAST_DUMMY_ITEM");
      return false;
    }

    if(checkAndconvertWriteData(value, data, unit, item_info) == false)
    {
      ESP_LOGE(TAG, "checkAndconvertWriteData failed");
      return false;
    }

    ret = writeControlTableItem(model_num, item_info.item_idx, id, data);
  }else{
    ESP_LOGI(TAG, "Unregistered model");
    setLastLibErrCode(D2A_LIB_ERROR_UNKNOWN_MODEL_NUMBER);
  }
...
static bool checkAndconvertWriteData(float in_data, int32_t &out_data, uint8_t unit, ItemAndRangeInfo_t &item_info)
{
  ...
  case UNIT_MILLI_AMPERE:
    if(unit != item_info.unit_type)
      return false;
    data = (int32_t)(in_data/item_info.unit_value);
